...
const { bootstrapDatabase } = require('./utils/bootstrap_database');
...
try {
  ...
  try {
    ...
    fallbackRouter.post(
      '/',
      timelyReflectionAdminMiddleware,
      timelyReflectionController.create,
    );
    fallbackRouter.delete(
      '/:id',
      timelyReflectionAdminMiddleware,
      timelyReflectionController.remove,
    );
...
logger.startup(`DB_URL    : ${!!process.env.DATABASE_URL}`);
...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    const icon =
      res.statusCode < 300 ? '->' : res.statusCode < 400 ? '~>' : 'x';
...
app.get('/health', async (req, res) => {
  ...
  res.json({
    success: true,
    message: 'Grace Church API is alive',
...
cron.schedule('*/2 * * * *', async () => {
  try {
    ...
    logger.error(`CRON retry error: ${error.message}`);
  }
});
...
cron.schedule('*/30 * * * *', async () => {
  ...
  try {
    ...
    logger.error(`CRON YT cache error: ${error.message}`);
  }
});
...
async function startServer() {
  ...
  try {
    const [databaseRows] = await db.promise().query(
      'SELECT current_database() AS db_name',
    );
    const activeDatabase = databaseRows[0]?.db_name || process.env.DB_NAME;

    logger.startup('Database connected');
    logger.startup(`Using database: ${activeDatabase}`);
    ...
    logger.startup(`users table OK - ${usersRows[0].cnt} user(s)`);

    app.listen(PORT, () => {
      logger.startup('='.repeat(60));
      logger.startup(`Server live on port ${PORT}`);
      logger.startup(`Database   : ${activeDatabase}`);
    ...
    logger.error(`Code: ${error.code || 'unknown'} | Message: ${error.message}`);
    logger.error(
      `Host: ${process.env.DB_HOST || 'n/a'}:${process.env.DB_PORT || 'n/a'} DB: ${process.env.DB_NAME || 'n/a'}`,
    );
    process.exit(1);
  }
}
